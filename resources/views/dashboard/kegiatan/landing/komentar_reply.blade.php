...
<div style="margin-left: 2cm;">
    ...
    <form method="post" class="collapse ml-5" id="collapseExample{{ $comment->id }}" action="{{ route('kegiatan.reply.store') }}">
        @csrf
        <div class="form-group">
            <input type="text" name="komentar" class="form-control" />
            <input type="hidden" name="id_kegiatan" value="{{ $comment->id_kegiatan }}" />
    ...
    @endif
    @include('dashboard.kegiatan.landing.komentar_reply', ['comments' => $comment->replies])
</div>
@endforeach